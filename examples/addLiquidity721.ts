...
const addLiquidity721 = async () => {
  ...
  const token721HasAllowance = await token721Contract.isApprovedForAll(
    signer.address,
    RouteErc721Address[SupportChain.Sepolia]
  )
  console.log('🚀 ~ token721HasAllowance:', token721HasAllowance)
  if (!token721HasAllowance) {
    const approve721Transaction = await token721Contract.setApprovalForAll(
      RouteErc721Address[SupportChain.Sepolia],
      true
    )
  ...
  if (bn_wrapperBn(token20Allowance).lt(buyPrice)) {
    const approve20Transaction = await token20Contract.approve(
      RouteErc721Address[SupportChain.Sepolia],
      bn_parseWei(9999999999).toString()
    )
